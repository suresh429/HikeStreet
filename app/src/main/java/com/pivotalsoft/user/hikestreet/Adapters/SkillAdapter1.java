...
public class SkillAdapter1 extends RecyclerView.Adapter<SkillAdapter1.MyViewHolder> {
    ...
    public SkillAdapter1(Context mContext, List<SkillsItem> coursesItemList) {
        this.mContext = mContext;
        this.coursesItemList = coursesItemList;
    }

    @Override
    public SkillAdapter1.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.exprince_card, parent, false);

        return new SkillAdapter1.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(final SkillAdapter1.MyViewHolder holder, final int position) {
       ...
       /* holder.parentLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent details =new Intent(mContext,EditSkillaActivity.class);
                details.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP|Intent.FLAG_ACTIVITY_NEW_TASK);
                details.putExtra("institute",album.getInstitute());
                details.putExtra("course",album.getCourse());
                details.putExtra("startyear",album.getStartyear());
                details.putExtra("endyear",album.getEndyear());
                details.putExtra("skillid",album.getSkillid());
